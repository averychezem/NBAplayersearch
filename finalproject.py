...
def makeObjectfromitem(item):
   ...
   return {'Position': tds[0].get_text()[:-1], 'Name':tds[2].get_text()[:-1], 'Number':tds[1].get_text()[20:-1], 'From':tds[6].get_text()[:-1]}
...
with open('NBA_info_Wikipedia.json', 'w') as file:
  json.dump(anyTeam, file)
...
with open('NBA_Info_Twitter.json', 'w') as file:
